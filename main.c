...
int run_compare(char *filename1,char *filename2){
    ...
    while (fgets(line1, MAX_LINE_LENGTH, file1) != NULL && fgets(line2, MAX_LINE_LENGTH, file2) != NULL) {
        ...
        if (strcmp(line1, line2) != 0) {
            flag_not_same=1;
            printf(RED_COLOR "«««««\n"RESET_COLOR);
            printf(RED_COLOR"%s-",filename1);
            //find line
            FILE*file3=fopen(filename1,"r");
            char line3[MAX_LINE_LENGTH];
            int num=1;
            while (fgets(line3, sizeof(line3), file3) != NULL) {
                ...
                //printf("%s %s",line3,line1);
                if(strcmp(line3,line1)==0){printf("%d\n"RESET_COLOR,num);}
                num++;
            }
            fclose(file3);
            printf(RED_COLOR"%s"RESET_COLOR, line1);
            printf(BLUE_COLOR"%s-",filename2);
            //find line
            FILE*file4=fopen(filename2,"r");
            char line4[MAX_LINE_LENGTH];
            num=1;
            while (fgets(line4, sizeof(line4), file4) != NULL) {
                int length = strlen(line4);
                // remove '\n'
                
                if(strcmp(line4,line2)==0){printf("%d\n"RESET_COLOR,num);}
                num++;
            }
            fclose(file4);
...
int run_diff_commit(int argc,char * const argv[]){
    ...
    printf("files that they are not same:\n");
    for(int k=0;k<i1;k++){
        int flag_same=0;
        for(int x=0;x<i2;x++){
            if(strcmp(name_1[k],name_2[x])==0){flag_same=1;}
    }
    if(flag_same==0){printf("%s\n",name_1[k]);}
    }
    for(int x=0;x<i1;x++){
        int flag_same=0;
        for(int k=0;k<i2;k++){
